...
class DatasetSegmentation(Dataset):
    ...
    def __init__(self, config_file: dict, processor: Samprocessor, mode: str):
        ...
        if mode == "train":
            ...
            self.img_files = glob.glob(os.path.join(config_file["DATASET"]["TEST_PATH"],'resized_image','*.png'))
            self.mask_files = []
            for img_path in self.img_files:
                self.mask_files.append(os.path.join(config_file["DATASET"]["TEST_PATH"],'final_mask', os.path.basename(img_path)[:-4] + ".png"))
    ...
    def __getitem__(self, index: int) -> list:
            ...
            # print("orginal size",original_size, type(original_size))
